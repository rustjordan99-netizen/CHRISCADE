...
/* update_freq() maintains freq_counter <= FREQ_INC_REF between calls.
 * Each true return advances pos by strictly less than freq_inc. The old
 * ((pos - prev_pos) / freq_inc) contribution was therefore always integer
 * zero, not fractional interpolation. Keep every phase/envelope transition
 * but omit those divisions. The ARM differential test compares PCM and all
 * APU state against the previous implementation. */
...
static void __time_critical_func(update_square)(int16_t* samples, const bool ch2)
{
	uint32_t freq;
	...
	c->freq_inc *= 8;
	const int32_t left_gain = c->on_left * vol_l;
	const int32_t right_gain = c->on_right * vol_r;
	...
	for (uint_fast16_t i = 0; i < AUDIO_NSAMPLES; i += 2) {
		...
		samples[i + 0] += sample * left_gain;
		samples[i + 1] += sample * right_gain;
	}
}
...
static void update_wave(int16_t *samples)
{
	...
	const int32_t left_gain = c->on_left * vol_l;
	const int32_t right_gain = c->on_right * vol_r;
	const unsigned output_shift = c->volume + 1u;
	...
	for (uint_fast16_t i = 0; i < AUDIO_NSAMPLES; i += 2) {
		...
		// Only the final phase contributes to this sample. Wave RAM and
		// routing are stable throughout this synchronous core-0 callback.
		c->wave.sample = wave_sample(c->val, c->volume);
		int32_t sample = ((int)c->wave.sample - 8) * (int)(INT16_MAX/64);
		...
		// Equivalent to / {1,2,4} then /4, including C++ truncation toward
		// zero for negative samples; no software divide or per-sample table.
		sample = sample < 0 ? -((-sample) >> output_shift) : sample >> output_shift;
 ...
 */
void __time_critical_func(audio_callback)(void *userdata, int16_t *stream, size_t len)
{
